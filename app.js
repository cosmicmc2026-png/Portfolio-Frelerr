(function () {
  ...
  function renderLanguageOptions() {
    ...
    languageSelect.innerHTML = `
      <button class="language-toggle" type="button" aria-expanded="false">
        <span>${escapeHtml(`${currentMeta.flag} ${currentMeta.label}`)}</span>
      </button>
      <div class="language-panel" hidden>
        ${Object.keys(data.translations).map((language) => {
          const meta = languageMeta(language);
          return `<button class="${language === currentLang ? "is-active" : ""}" type="button" data-language="${escapeHtml(language)}">${escapeHtml(`${meta.flag} ${meta.label}`)}</button>`;
        }).join("")}
      </div>
    `;
  ...
  function renderProjects() {
    ...
    grid.innerHTML = projects.map((project, index) => `
      <article class="project-card reveal" style="--accent:${safeColor(project.accent)}; --project-bg:${projectBackground(project, index)}">
        <div class="project-visual">
          <div class="project-icon" aria-hidden="true">${projectIcon(project.category)}</div>
        ...
        <div class="project-content">
          ...
          ${project.url && project.url !== "#" ? `<a class="project-link" href="${escapeHtml(safeUrl(project.url))}" target="_blank" rel="noreferrer">${escapeHtml(translate("openProject"))}</a>` : ""}
        </div>
      </article>
    `).join("");

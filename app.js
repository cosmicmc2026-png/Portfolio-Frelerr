(function () {
  ...
  function renderLanguageChoice(meta, options = {}) {
    const showLabel = options.showLabel !== false;
    return `
      <span class="language-choice">
        <span class="language-flag" aria-hidden="true">
          ${meta.flagSrc ? `<img src="${escapeHtml(meta.flagSrc)}" alt="">` : ""}
        </span>
        ${showLabel ? `<span class="language-label">${escapeHtml(meta.label)}</span>` : ""}
      </span>
    `;
  }
  ...
  function renderTopNav(activeCategory) {
    if (!navLinks) return;
    const previousState = navLinks.dataset.navState || "";
    const nextState = activeCategory || "home";
    const changedState = previousState && previousState !== nextState;
    navLinks.dataset.navState = nextState;
  ...
  function animateTopNav() {
    ...
    if (!topbar) return;
    topbar.classList.remove("is-nav-changing");
    void topbar.offsetWidth;
    topbar.classList.add("is-nav-changing");
    window.setTimeout(() => topbar.classList.remove("is-nav-changing"), 420);
  }
  ...
  function renderLanguageOptions() {
    ...
    languageSelect.innerHTML = `
      <button class="language-toggle" type="button" aria-label="${escapeHtml(currentMeta.label)}" aria-expanded="false">
        ${renderLanguageChoice(currentMeta, { showLabel: false })}
      </button>
